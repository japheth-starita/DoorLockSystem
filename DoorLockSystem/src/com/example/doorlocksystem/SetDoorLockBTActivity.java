	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        option.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (deviceName.contains("DOORLOCK")){
					SharedPreferences.Editor editor = getSharedPreferences(MainActivity.PREFS_NAME, 0).edit();
				    editor.putString("btname", deviceName);
				    editor.commit();
				    Toast.makeText(getApplicationContext(), deviceName + "Selected", Toast.LENGTH_LONG).show();
				    finish();
				}
				else{
					Toast.makeText(getApplicationContext(), "Invalid Door Lock Bluetooth Device Name", Toast.LENGTH_LONG).show();
				}
			}
        }); 
	}
